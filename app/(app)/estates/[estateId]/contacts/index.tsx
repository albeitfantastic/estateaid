  ...
  return (
    <ThemedView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <TouchableOpacity style={[styles.addBtn, { backgroundColor: colors.tint }]} onPress={() => router.push(`/(app)/estates/${estateId}/contacts/new` as never)}>
          <IconSymbol name="plus" size={20} color="#fff" />
        </TouchableOpacity>
      </View>

      {contacts.length === 0 ? (
        <EmptyState icon="phone.fill" title="No contacts yet" subtitle="Add important numbers for your guests." actionLabel="Add Contact" onAction={() => router.push(`/(app)/estates/${estateId}/contacts/new` as never)} />
      ) : (
              ...
              {catContacts.map((contact) => (
                <View key={contact.id} style={[styles.row, { borderColor: colors.icon + '22', backgroundColor: colors.background }]}>
                  ...
                  <View style={styles.actions}>
                    ...
                    <TouchableOpacity onPress={() => router.push(`/(app)/estates/${estateId}/contacts/${contact.id}/edit` as never)}>
